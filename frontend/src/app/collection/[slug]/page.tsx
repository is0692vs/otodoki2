  ...
  useEffect(() => {
    ...
    const fetchTracks = async () => {
      ...
      try {
        let trackData: Track[] = [];
        if (slug === "liked") {
          const response = await api.tracks.liked();
          if (response.error) {
            setError(response.error.error);
          } else {
            trackData = response.data?.tracks || [];
          }
        } else if (slug === "disliked") {
          // TODO: disliked APIはIDリストしか返さないので、Trackデータを取得する方法を実装
          // 現在は空の配列を返す
          const response = await api.tracks.disliked();
          if (response.error) {
            setError(response.error.error);
          } else {
            // disliked_idsからTrackデータを取得する方法を実装する必要がある
            // 現在は空の配列を返す
            trackData = [];
            console.log("📋 Disliked track IDs:", response.data?.disliked_ids);
          }
        } else {
