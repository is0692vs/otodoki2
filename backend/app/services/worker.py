...
class QueueReplenishmentWorker:
    ...
    async def trigger_refill(self) -> bool:
        ...
        if self._refill_lock.locked():
            logger.info("Refill already in progress, skipping one-shot trigger")
            return False
        ...
        while self._running:
            try:
                ...
                if current_size < min_threshold:
                    logger.info(f"Queue size ({current_size}) below threshold ({min_threshold}), attempting refill")
                    ...
                    logger.debug(f"Queue size ({current_size}) above threshold ({min_threshold}), no refill needed")
    ...
    async def _attempt_refill(self) -> bool:
        ...
        try:
            ...
            if need <= 0:
                logger.info(f"Queue is at capacity ({current_size}/{max_cap}), no refill needed")
                return True
            ...
            while filled < need and attempts < max_attempts:
                try:
                    # キーワード選択
                    term = self.itunes_client.pick_search_term()
                    
                    # iTunes API検索
                    raw_tracks = await self.itunes_client.search_tracks(term, limit=200)
                    if not raw_tracks:
                        attempts += 1
                        continue
                    
                    # データ整形
                    cleaned_tracks = self.itunes_client.clean_and_filter_tracks(raw_tracks)
                    ...
                    logger.info(f"Added {added_count} tracks to queue (total filled: {filled}/{need})")
                    ...
                    logger.warning(f"Failed to fetch tracks with term '{term}': {e}")
            ...
            if filled > 0:
                logger.info(f"Refill completed: added {filled} tracks, queue size: {current_size} -> {final_size}")
                return True
            else:
                logger.warning(f"Refill failed: no tracks added after {attempts} attempts")
                return False
    ...
    def _should_skip_due_to_failures(self) -> bool:
        ...
        # バックオフ時間の計算
        backoff_time = (self._failure_backoff_multiplier ** min(self._consecutive_failures - self._max_failures, 5)) * 60
        time_since_failure = time.time() - self._last_failure_time
        
        if time_since_failure < backoff_time:
            logger.debug(f"Skipping refill due to consecutive failures (backoff: {backoff_time:.1f}s)")
            return True
