...
class iTunesApiClient:
    ...
    def __init__(self):
        ...
        # キーワードのクールダウン管理
        self._keyword_cooldown: Dict[str, datetime] = {}
        self._cooldown_duration = timedelta(minutes=5)  # 5分間のクールダウン
    ...
    def pick_search_term(self) -> str:
        ...
        # クールダウン中でないキーワードをフィルタリング
        for term in self.config.get_itunes_terms():
            last_used = self._keyword_cooldown.get(term)
            if last_used is None or (current_time - last_used) >= self._cooldown_duration:
                available_terms.append(term)
        
        # 利用可能なキーワードがない場合は全て対象に
        if not available_terms:
            available_terms = self.config.get_itunes_terms()
            logger.info("All keywords in cooldown, resetting cooldown period")
        
        # ランダム選択
        selected_term = random.choice(available_terms)
        self._keyword_cooldown[selected_term] = current_time
