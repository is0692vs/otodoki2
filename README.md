...
│   └── shared/        # Web/バックエンドで共有する TypeScript SDK
├── docs/              # 設計ドキュメント
├── scripts/           # 開発・検証スクリプト
...
依存関係はルートディレクトリで `pnpm install` を実行してインストールします（初回のみ）。

```bash
pnpm install
```

Dev Containers で開くと pnpm や Node.js などが揃った状態で作業を開始できます。
...
停止は `docker compose down` または `make down` を利用してください。フロントエンドのみローカルで動かす場合は `pnpm --filter web dev` を使用します。
...
| `OTODOKI_*`                        | iTunes 検索戦略やワーカーの閾値を調整する環境変数群。詳細は `packages/backend/app/core/config.py` を参照してください。                                 | -                  |
...
- **テスト**: Docker 上の PostgreSQL と連携したまま実行するには以下を推奨します。
  ...
  PYTHONPATH=./packages/backend pytest packages/backend/tests/
  ```
...
- **pytest が失敗する**: `DATABASE_URL` が PostgreSQL を指しているか、`pip install -r packages/backend/requirements.txt` が完了しているかを確認してください。
- **フロントとバックを別々に起動する場合**: バックエンドは `cd packages/backend && uvicorn app.main:app --reload`、フロントエンドは `pnpm --filter web dev` で起動します。この場合でも `.env` の `NEXT_PUBLIC_API_URL` がバックエンドを指している必要があります。
